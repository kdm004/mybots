...
# Removed global linkNamesToIndices
def End():
...
def Get_Touch_Sensor_Value_For_Link(linkName, linkNamesToIndices):
...
def Prepare_Joint_Dictionary(bodyID):
    ...
    return jointNamesToIndices  # Return the dictionary

def Prepare_To_Simulate(bodyID):
    linkNamesToIndices = Prepare_Link_Dictionary(bodyID)
    jointNamesToIndices = Prepare_Joint_Dictionary(bodyID)
    return linkNamesToIndices, jointNamesToIndices  # Return both dictionaries

def Send_Cube(name="default", pos=[0, 0, 0], size=[1, 1, 1], linkNamesToIndices=None):
...
def Set_Motor_For_Joint(bodyIndex, jointName, controlMode, targetPosition, maxForce, jointNamesToIndices):
