...
def plot_files_in_folder(folder, color, folder_index):
    for i in range(10):
        ...
        for position_file in position_files:
            ...
            if (folder_index == 2 and i != 4) or (folder_index == 3 and i != 2):
...
plt.figure(figsize=(5, 8.8))  # Set figure size to 500x880
...
plt.xlim(-8, 8)   # x limits
plt.ylim(-14, 10) # y limits
plt.gca().invert_yaxis()  # Invert y axis
# plt.title('Plot of Trajectories')

# Legend only for solid lines
solid_patches = [plt.Line2D([], [], color=colors[i-1], label=f'Case {i}') for i in range(1, len(folders)+1)]
plt.legend(handles=solid_patches)
